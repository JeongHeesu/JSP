...
<body>
<form name="memberViewForm" method="post" action="<%=request.getContextPath()%>/06/updateMemberInfo.jsp">
...
$(function(){
	...
	//메일주소
	var mail = '<%=memberInfo.getMem_mail() %>';
	$('input[name=mem_mail1]').val(mail.split("@")[0]);
	$('select[name=mem_mail2] option[value="'+mail.split("@")[1]+'"]').attr('selected', true);
	//우편번호
	...
	$('form[name=memberViewForm]').submit(function(){
		...
		if(!$('input[name=mem_pass]').val().validationID()){
	...
	//삭제
	$('#btn3').click(function(){
		var $form= $('<form mehtod="POST" action="/ddit/06/deleteMemberInfo.jsp"></form>');
